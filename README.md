...
6.  3D audio content access: Provides the capability to query the multi-channel data formats supported by the device for playback (such as channel configuration, encoding format, and sample rate) and to listen for format changes. Applications can use this to select an appropriate multi-channel format and play multi-channel data.
